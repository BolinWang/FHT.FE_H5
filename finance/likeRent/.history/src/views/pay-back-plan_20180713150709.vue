        ...
        <van-col span="24" class="list" @click.native="toDetail(index, item)"   v-for="(item, index) in planList" :key="index">
          <div class="list-top van-hairline--bottom">
            <van-row>
              ...
              <van-col span="6">租金金额 {{feeChange(item.repayAmt)}}</van-col>
              <van-col span="6"  offset="8" class="status" :class="{active: payStatus == 1}">
          ...
          <div class="list-top">
            <van-row>
              <van-col span="12" class="time">还款时间 {{item.repayDate}}</van-col>
              <van-col span="1"  offset="10">></van-col>
            </van-row>
          </div>
