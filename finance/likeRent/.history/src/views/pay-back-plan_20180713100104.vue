<template>
  <div class="container">
    ...
    />
    <van-row>
      <router-link :to="{ name: 'detail', params: { externalOrderNo: externalOrderNo, periods: item.refundPeriods }}"  v-for="(item, index) in planList" :key="index" >
...
export default {
  ...
  methods: {
    toDetail () {
      console.log(index)
      if (index !== 0) {
        this.$toast('暂不支持跨期付款哦~')
        return
      }
      this.$router.push({name: 'detail', params: { externalOrderNo: this.billDetail.externalOrderNo, periods: this.billDetail.refundPeriods, fee: this.billDetail.repayAmt }})
      this.$router.go()
    }
  },
  mounted () {
    ...
    getPayPlanList(externalOrderNo).then(res => {
      ...
      this.periods = res.data.refundPeriods
    })
  }
}
</script>
