      ...
      <div class="infomation_top">
        <div class="top_container">
          ...
          <div class="houseInfo_slider" v-show="showHouseInfo">
            ...
            <div class="slider_items flex flex-center">
              <div class="slider_item">
                户型：{{houseInfo.houseType}}
              </div>
              <div class="slider_item">
                装修：{{houseInfo.decorationDegree | filterDegree}}
              </div>
            </div>
            <div class="slider_items flex flex-center">
              <div class="slider_item">
                面积：{{houseInfo.houseArea}}㎡
              </div>
              <div class="slider_item">
                朝向：{{houseInfo.roomDirection | filterDirection}}
              </div>
            </div>
            <div class="slider_items flex flex-center">
              <div class="slider_item">
                楼层：{{houseInfo.floorName}}
              </div>
              <div class="slider_item">
                编号：{{houseInfo.roomCode}}
              </div>
            </div>
          </div>
        </div>
    ...
    list:[
      {
        ...
        name:"客卫",
        previewUrl:"http://media.csimum.com/pano90f0bbef429bcb352fe92ea3b95d63da/thumb/500_500/panofile.jpg",
...
export default {
  ...
  filters: {
    filterDegree (val) {
      let filterData = ['毛坯', '简装', '精装修', '豪华装']
      return filterData[val - 1] || '-'
    },
    filterDirection (val) {
      let filterData = ['朝南', '朝北', '朝东', '朝西', '东南', '西南', '东北', '西北']
      return filterData[val - 1] || '-'
    },
  },
  data () {
  ...
  methods: {
    ...
    renderVrPlayer (scenesData) {
      ...
      if (scenesData.modelType === 'pano') {
        defaultOptions.scenes = mapList
      } else if (scenesData.modelType === '25d' || scenesData.modelType === '25d+') {
        defaultOptions.scenes = mapList
        defaultOptions.model = scenesData['model' + scenesData.modelType]
        this.isShow = true
      } else if (scenesData.modelType === '3d') {
        defaultOptions.cover = scenesData.cover
        defaultOptions.model_3d = scenesData.model3d
        this.isShow = true
      } else {
