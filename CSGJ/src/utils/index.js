 ...
 * @Last Modified time: 2018-05-23 15:02:57
 */
...
export function validResult(dom) {
  let flag = true
  dom.map((val, index) => {
    if (!val.valid) {
      val.focus()
...
export function valFormat(source, type) {
  if (!source && typeof source !== 'object') {
    return
  }
  const targetObj = source.constructor === Array ? [] : {}
  for (const keys in source) {
    if (source.hasOwnProperty(keys)) {
      targetObj[keys] = type === 'number' ? Number(source[keys]) : String(source[keys])
    }
  }
  return targetObj
}
