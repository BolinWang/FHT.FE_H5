<template>
  <div class="height100">
    <view-box ref="viewBox" body-padding-top="46px" >
      <x-header title="新增房东" slot="header" style="width:100%;position:absolute;left:0;top:0;z-index:100;">
      </x-header>
      <group class="noTop" label-width="80px">
        <x-input title="姓名" required v-model="userForm.name" placeholder="请输入">
          <i class="iconfont icon-zhaoxiangji" slot="right" @click.prevent="photo"></i>
        </x-input>
        ...
        <x-textarea title="备注" :rows="1" placeholder="最多10个字" v-model="userForm.desc" :max="10" autosize :show-counter="false"></x-textarea>
        <cell title="身份证照片">
          <img-upload :uploadParam="uploadParam"></img-upload>
        </cell>
      </group>
      <div slot="bottom" class="bottomDiv">
        <x-button type="primary" action-type="button" @click.native="toSave">提交申请</x-button>
      </div>
    </view-box>
  </div>
</template>
